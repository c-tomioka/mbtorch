...
- **2026-03**: Phase 1 MVP 実装完了。core/tensor（add/sub/mul/scale/matmul 等）、core/autograd（Variable + backward + grad_check）、nn（Linear / 2層MLP）、optim（SGD）、examples（basic_tensor_ops / basic_autograd / basic_mlp）。67テスト全 PASS。次は io（`.mbt` / ONNX / safetensors）に進む。
