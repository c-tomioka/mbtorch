...
## 2. 設計決定を記録する（adr-madr）

ステップ 1 で重要なアーキテクチャ判断を行った場合は、その決定を ADR（Architectural Decision Record）として記録します。
すべての変更に必要なわけではなく、**後から「なぜこうしたのか」を振り返りたくなるような判断**が対象です。

```text
/adr-madr
core/tensor に基本テンソル演算を追加するにあたり、
演算を ops_basic.mbt に集約する方針を決めました。
検討した案:
- 演算ごとに別ファイル（ops_add.mbt, ops_mul.mbt, ...）
- 1 ファイルに全演算をまとめる（ops_basic.mbt）
- trait ベースで演算を定義する
最終的に ops_basic.mbt にまとめる案を採用しました。
理由: ファイル数の爆発を避けつつ、将来的な分割も容易なため。
```

### git log や PR から逆算して ADR を起こしたいとき

過去の決定を後追いで記録する場合は、関連コミットや PR の情報を渡します。

```text
/adr-madr
以下の git log から、設計決定を ADR として記録してください。
推測が必要な部分は (assumption) と明記してください。
...
- ADR ファイルは `docs/adr/` に配置する（命名: `0001-kebab-case-title.md`）
- 1 ファイル 1 決定を厳守する
- 情報が不足している場合は `(assumption)` や `(要確認)` を明記する

***

## 3. 先にテストを書く（mbtorch-tdd）
...
2. （重要な判断があれば）`/adr-madr`
   → 設計決定を ADR として記録する

3. `/plan + /mbtorch-tdd`
   → テストファイル＋テストコードを書く（実装はまだ）

4. （必要なら）`/plan + /mbtorch-api-design`
   → API 形状とサンプルコードを固める

5. `/plan + /mbtorch-tdd`（＋ `/moonbit-evaluating-code` / `/moonbit-investigating-errors`）
   → 実装してテストを通す

6. `/plan + /mbtorch-docs` ＋ `/plan + /mbtorch-examples`
   → README / examples を更新する

7. 規模が大きくなってきたら `/plan + mbtorch-refactor`
   → 構造の整理・分割を行う  
